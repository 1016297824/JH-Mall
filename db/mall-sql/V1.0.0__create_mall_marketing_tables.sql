...
CREATE TABLE IF NOT EXISTS `mall_marketing_coupon` (
    ...
    `face_value`       bigint unsigned NOT NULL                        COMMENT '优惠面值（单位：分）',
    `discount_rate`    tinyint unsigned DEFAULT NULL                   COMMENT '折扣率（百分比）',
    `discount_limit`   bigint unsigned DEFAULT NULL                    COMMENT '折扣上限（单位：分）',
    `min_order_amount` bigint unsigned DEFAULT 0                       COMMENT '最低订单金额门槛（单位：分）',
    `total_count`      int unsigned    NOT NULL                        COMMENT '发行总量',
...
CREATE TABLE IF NOT EXISTS `mall_marketing_promotion_rule` (
    ...
    `benefit_amount`   bigint unsigned DEFAULT NULL                    COMMENT '优惠金额（单位：分）',
    `benefit_rate`     tinyint unsigned DEFAULT NULL                   COMMENT '折扣率',
    `is_exclusive`     tinyint unsigned DEFAULT 0                      COMMENT '是否互斥',
    `sort_order`       int unsigned    DEFAULT 0                       COMMENT '优先级',
    `is_deleted`       tinyint unsigned DEFAULT 0                      COMMENT '逻辑删除标志',
    `create_time`      datetime        NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time`      datetime        NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
    PRIMARY KEY (`id`),
