...
CREATE TABLE IF NOT EXISTS `mall_order_cart` (
    ...
    `is_selected` tinyint unsigned DEFAULT 1                      COMMENT '是否选中：1=选中 0=未选中',
...
CREATE TABLE IF NOT EXISTS `mall_order` (
    ...
    `user_id`         bigint unsigned NOT NULL                        COMMENT '用户 ID',
    `order_status`    tinyint unsigned NOT NULL                       COMMENT '订单状态：1=待支付 2=已支付 3=待发货 4=待收货 5=已完成 6=已取消 7=已关闭 8=退款中 9=已退款',
    ...
    `cancel_type`     varchar(20)     DEFAULT NULL                    COMMENT '取消类型：user_cancel / timeout_cancel / admin_cancel',
    `cancel_reason`   varchar(200)    DEFAULT NULL                    COMMENT '取消原因',
    `pay_expire_time` datetime        NOT NULL                        COMMENT '支付过期时间，默认创建后 30 分钟',
    `remark`          varchar(200)    DEFAULT NULL                    COMMENT '买家备注',
    `idempotent_key`  varchar(64)     DEFAULT NULL                    COMMENT '幂等键：userId_clientRequestNo',
    `is_deleted`      tinyint unsigned DEFAULT 0                      COMMENT '逻辑删除标志',
...
CREATE TABLE IF NOT EXISTS `mall_order_after_sale` (
    ...
    `after_sale_type`        tinyint unsigned NOT NULL                       COMMENT '售后类型：1=仅退款 2=退货退款 3=换货',
    `reason`                 varchar(200)    DEFAULT NULL                    COMMENT '退款原因',
    `amount`                 bigint unsigned DEFAULT NULL                    COMMENT '退款金额（单位：分）',
    `after_sale_status`      tinyint unsigned NOT NULL DEFAULT 0             COMMENT '售后状态：0=待审核 1=审核通过 2=审核驳回 3=买家已退货 4=商家已收货 5=退款中 6=退款完成 7=已关闭',
...
CREATE TABLE IF NOT EXISTS `mall_outbox` (
    ...
    `topic`           varchar(128) NOT NULL                            COMMENT '消息主题，如 mall:order:created',
