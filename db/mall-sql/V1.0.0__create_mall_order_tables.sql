...
CREATE TABLE IF NOT EXISTS `mall_order_cart` (
    ...
    `sku_name`    varchar(200)    DEFAULT NULL                    COMMENT 'SKU 销售名称',
    `main_image`  varchar(500)    DEFAULT NULL                    COMMENT '商品主图 URL',
    `price`       bigint unsigned NOT NULL                        COMMENT '当前销售价（单位：分）',
...
CREATE TABLE IF NOT EXISTS `mall_order` (
    ...
    `pay_expire_time` datetime        NOT NULL                        COMMENT '支付过期时间',
...
CREATE TABLE IF NOT EXISTS `mall_order_item` (
    ...
    `total_price` bigint unsigned NOT NULL                        COMMENT '单项总价（单位：分）',
    `is_deleted`  tinyint unsigned DEFAULT 0                      COMMENT '逻辑删除标志',
    `create_time` datetime        NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time` datetime        NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
    PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `mall_order_after_sale` (
    ...
    `order_item_id`          bigint unsigned DEFAULT NULL                    COMMENT '关联订单项 ID',
