...
CREATE TABLE IF NOT EXISTS `mall_product_category` (
    ...
    `parent_id`   bigint unsigned NOT NULL DEFAULT 0              COMMENT '父类目 ID',
    ...
    `path`        varchar(255)    DEFAULT NULL                    COMMENT '类目路径',
...
CREATE TABLE IF NOT EXISTS `mall_product_spu` (
    ...
    `spu_description` text            DEFAULT NULL                    COMMENT '商品详情描述',
...
CREATE TABLE IF NOT EXISTS `mall_product_sku` (
    ...
    `attrs_json`   text            DEFAULT NULL                    COMMENT '销售属性 JSON',
    ...
    `weight`       int unsigned    DEFAULT 0                       COMMENT '重量（单位：克）',
    `sales_count`  int unsigned    DEFAULT 0                       COMMENT '累计销量',
    `is_deleted`   tinyint unsigned DEFAULT 0                      COMMENT '逻辑删除标志',
    `create_time`  datetime        NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time`  datetime        NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间',
