...
CREATE TABLE IF NOT EXISTS `mall_user` (
    ...
    `phone`               varchar(20)     NOT NULL                             COMMENT '手机号（AES-256-GCM 加密存储）',
    `phone_hash`          varchar(64)     NOT NULL                             COMMENT '手机号 SHA256 哈希辅助列，用于等值查询',
    `password`            varchar(256)    NOT NULL                             COMMENT '密码 BCrypt 哈希',
    `nickname`            varchar(50)     DEFAULT NULL                         COMMENT '昵称',
    `avatar`              varchar(500)    DEFAULT NULL                         COMMENT '头像 URL',
    `email`               varchar(100)    DEFAULT NULL                         COMMENT '邮箱（AES-256-GCM 加密存储）',
    `email_hash`          varchar(64)     DEFAULT NULL                         COMMENT '邮箱 SHA256 哈希辅助列',
    `gender`              tinyint unsigned DEFAULT 0                           COMMENT '性别：0=未知 1=男 2=女',
    `birthday`            date            DEFAULT NULL                         COMMENT '生日',
    `user_status`         tinyint unsigned DEFAULT 0                           COMMENT '用户状态：0=正常 1=冻结 2=注销',
    `register_type`       varchar(20)     DEFAULT NULL                         COMMENT '注册方式：phone / wechat / email',
    ...
    `is_privacy_agreed`   tinyint unsigned DEFAULT 0                           COMMENT '是否同意隐私协议：1=同意 0=未同意',
    ...
    `is_deleted`          tinyint unsigned DEFAULT 0                           COMMENT '逻辑删除标志：1=已删除 0=未删除',
...
CREATE TABLE IF NOT EXISTS `mall_user_member_level` (
    ...
    `level_value`   tinyint unsigned NOT NULL               COMMENT '等级值，数值越大等级越高',
    ...
    `benefits_json` text            DEFAULT NULL            COMMENT '权益 JSON，如折扣率、免邮费、积分倍数',
...
CREATE TABLE IF NOT EXISTS `mall_user_member` (
    `id`               bigint unsigned NOT NULL AUTO_INCREMENT COMMENT '主键，自增',
    `user_id`          bigint unsigned NOT NULL                COMMENT '用户 ID，与 mall_user 一对一',
    `level_id`         bigint unsigned DEFAULT NULL            COMMENT '当前会员等级 ID，关联 mall_user_member_level',
    `growth`           int unsigned    DEFAULT 0               COMMENT '当前成长值',
    `total_growth`     int unsigned    DEFAULT 0               COMMENT '累计获取成长值，仅增不减',
...
CREATE TABLE IF NOT EXISTS `mall_user_address` (
    ...
    `receiver_phone` varchar(20)     NOT NULL                COMMENT '收件人手机号（AES-256-GCM 加密存储）',
    `province`       varchar(50)     DEFAULT NULL            COMMENT '省',
    `city`           varchar(50)     DEFAULT NULL            COMMENT '市',
    `district`       varchar(50)     DEFAULT NULL            COMMENT '区',
    `detail_address` varchar(200)    DEFAULT NULL            COMMENT '详细地址',
    `zip_code`       varchar(10)     DEFAULT NULL            COMMENT '邮编',
    `is_default`     tinyint unsigned DEFAULT 0              COMMENT '是否默认地址：1=默认 0=非默认',
...
CREATE TABLE IF NOT EXISTS `mall_user_points_account` (
    `id`               bigint unsigned NOT NULL AUTO_INCREMENT COMMENT '主键，自增',
    `user_id`          bigint unsigned NOT NULL                COMMENT '用户 ID，与用户一对一',
    `total_points`     int unsigned    DEFAULT 0               COMMENT '累计获取积分，仅增不减',
...
CREATE TABLE IF NOT EXISTS `mall_user_points_log` (
    `id`            bigint unsigned NOT NULL AUTO_INCREMENT COMMENT '主键，自增',
    `user_id`       bigint unsigned NOT NULL                COMMENT '用户 ID',
    `biz_type`      varchar(50)     NOT NULL                COMMENT '业务类型：order（下单）/ signin（签到）/ refund（退款）/ admin（管理员调整）',
    `biz_no`        varchar(64)     DEFAULT NULL            COMMENT '业务单号',
    `change_type`   tinyint unsigned NOT NULL               COMMENT '变动方向：1=增加 2=减少',
