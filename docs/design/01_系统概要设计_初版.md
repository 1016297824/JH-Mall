...
└─ server/                    # 🔧 所有后端服务
   ...
   └─ mall/                   # 🛒 商城业务域（完全独立，与 manager 平级）
     ...
     │     ├─ mapper/        # MyBatis-Plus 数据访问层
      ...
      │     ├─ controller/    # 控制层
      │     ├─ DO/            # MallOrderDO, MallOrderItemDO
     │     ├─ DTO/           # 本服务 Controller 入参
     │     ├─ VO/            # 本服务返回给前端的数据结构
     │     ├─ service/       # 业务逻辑层
     │     ├─ mapper/        # MyBatis-Plus 数据访问层
     ...
     │     ├─ mapper/        # MyBatis-Plus 数据访问层
      ...
      │     ├─ controller/    # 控制层
      │     ├─ DO/            # MallCouponDO, MallCouponRecordDO
     │     ├─ DTO/           # 本服务 Controller 入参
     │     ├─ VO/            # 本服务返回给前端的数据结构
     │     ├─ service/       # 业务逻辑层
     │     ├─ mapper/        # MyBatis-Plus 数据访问层
...
| 样板代码      | Lombok（`@Data` / `@Slf4j` / `@Builder` / `@RequiredArgsConstructor`），详见 AGENTS.md §Lombok 使用规范 |
