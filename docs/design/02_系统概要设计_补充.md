...
- 任何 `domain` 实体不能放入 `mall-api` 或 `ruoyi-common-*`
...
| 搜索引擎      | Elasticsearch 8.x + Spring Data Elasticsearch（mall-search）                |
| 消息队列      | RocketMQ 5.x + ruoyi-common-stream（消息模型见 12.4）                     |
...
└─ server/                    # 🔧 所有后端服务
   ...
   │  │
   │  ├─ ruoyi-auth/          # 管理端专属认证中心（9200端口，仅管理员登录）
   │  │
   ...
   │  │  ├─ ruoyi-system/     # 管理端系统模块（9201端口：管理员、角色、权限、菜单）
   │  │  ├─ ruoyi-gen/        # ⚙️ 代码生成器（9202端口，生产环境删除）
   │  │  ├─ ruoyi-job/        # ✅ 全局复用：分布式定时任务（9203端口）
   │  │  └─ ruoyi-file/       # ✅ 全局复用：统一文件服务（9300端口）
   │  │
   │  └─ ruoyi-visual/        # 若依可视化模块
   │     └─ ruoyi-visual-monitor/ # ✅ 全局复用：服务监控中心（9100端口）
   │
   └─ mall/                   # 🛒 商城业务域（完全独立，与 manager 平级）
      ...
      │
      ├─ mall-product/        # 商城商品服务（9302端口）
      │  └─ src/main/java/com/jhstore/mall/product/
      │     ├─ controller/    # admin/ + api/ 控制层
      │     ├─ domain/        # MallProductDO, MallProductSkuDO, MallProductCategoryDO
      │     ├─ service/       # 业务逻辑层
      │     ├─ mapper/        # MyBatis 数据访问层
      ...
      │
      ├─ mall-order/          # 商城订单服务（9303端口）
      │  └─ src/main/java/com/jhstore/mall/order/
      │     ├─ controller/    # admin/ + api/ 控制层
      │     ├─ domain/        # MallOrderDO, MallOrderItemDO, MallCartDO, MallAfterSaleDO
      │     ├─ service/       # 业务逻辑层
      │     ├─ mapper/        # MyBatis 数据访问层
      │     ├─ statemachine/  # 订单状态机
      │     ├─ infrastructure/ # MQ、外部 Feign 适配
      ...
      │
      ├─ mall-payment/        # 商城支付服务（9304端口）
      │  └─ src/main/java/com/jhstore/mall/payment/
      │     ├─ controller/    # admin/ + api/ 控制层
      │     ├─ domain/        # MallPaymentDO, MallRefundDO, MallPaymentChannelDO
      │     ├─ service/       # 业务逻辑层
      │     ├─ mapper/        # MyBatis 数据访问层
      │     ├─ statemachine/  # 支付状态机
      ...
      │
      ├─ mall-marketing/      # 商城营销服务（9305端口）
      │  └─ src/main/java/com/jhstore/mall/marketing/
      │     ├─ controller/    # admin/ + api/ 控制层
      │     ├─ domain/        # MallCouponDO, MallCouponRecordDO, MallPromotionDO
      │     ├─ service/       # 业务逻辑层
      │     ├─ mapper/        # MyBatis 数据访问层
      ...
      │
      └─ mall-search/         # 商城搜索服务（9306端口，基于 Elasticsearch）
         └─ src/main/java/com/jhstore/mall/search/
            ├─ controller/    # api/ 搜索接口
            ├─ domain/        # ES 索引实体（ProductIndex，ES 非 MySQL，不加 DO）
            ├─ service/       # 业务逻辑层
...
flowchart LR
    ...
    GW --> RuoyiAuth["ruoyi-auth :9200"]
    GW --> RuoyiSystem["ruoyi-system :9201"]
    GW --> MallAuth["mall-auth :9210"]
    GW --> User["mall-user :9301"]
    GW --> Product["mall-product :9302"]
    GW --> Order["mall-order :9303"]
    GW --> Payment["mall-payment :9304"]
    GW --> Marketing["mall-marketing :9305"]
    GW --> Search["mall-search :9306"]
...
| **两套认证中心**      | `ruoyi-auth(9200)` 管管理员登录，`mall-auth(9210)` 管 C 端用户登录。网关层通过 `AuthFilter` + `MallAuthFilter` 按路由前缀分流。两套 Token 体系隔离，符合若依管理端与商城业务解耦的目标                                                       |
...
| **用户与会员**        | 合并在 `mall-user(9301)` 一个服务内，不拆分。因为商城用户天然就是会员，拆分过度会增加服务间调用的复杂度                                                                                                                                            |
...
| **搜索服务**          | `mall-search(9306)` 基于 Elasticsearch 独立部署。商品索引通过 `mall-product` 在商品变更时调用 Feign 接口 `RemoteSearchService` 触发增量同步；全量重建由 `mall-search` 自身维护。C 端搜索走 `mall-search` 的 `api/` 接口，不走数据库 LIKE |
| **支付服务**          | `mall-payment(9304)` 独立部署，具体交互方式（同步 Feign / 异步 MQ / 回调处理）待详细设计时确定                                                                                                                                                     |
| **包名**              | 若依原生：`com.ruoyi`；商城业务：`com.jhstore.mall.xxx`。两者通过 Nacos 中配置的 `spring.application.name` 进行服务发现和 Feign 调用                                                                                                           |
| **网关位置**          | 复用 `ruoyi-gateway`，不新增独立网关                                                                                                                                                                                                               |
| **全局公共能力**      | 复用 `ruoyi-common-*`，不重复建设 `common/`                                                                                                                                                                                                      |
| **商城模块位置**      | 放在 `server/mall/`，与 `server/manager/` 平级，物理隔离                                                                                                                                                                                         |
| **管理端与 C 端认证** | 两套 token、两套链路，用户模型和风险策略不同                                                                                                                                                                                                         |
| **数据实体归属**      | `domain/` 拆到各微服务模块，避免跨服务表模型耦合                                                                                                                                                                                                   |
...
| ruoyi-auth（管理端认证）     | 9200 | manager |
| ruoyi-system（系统模块）     | 9201 | manager |
| ruoyi-gen（代码生成）        | 9202 | manager |
| ruoyi-job（定时任务）        | 9203 | manager |
| ruoyi-visual-monitor（监控） | 9100 | manager |
| ruoyi-file（文件服务）       | 9300 | manager |
| mall-auth（商城认证）        | 9210 | mall    |
| mall-user（用户会员）        | 9301 | mall    |
| mall-product（商品）         | 9302 | mall    |
| mall-order（订单）           | 9303 | mall    |
| mall-payment（支付）         | 9304 | mall    |
| mall-marketing（营销）       | 9305 | mall    |
| mall-search（搜索）          | 9306 | mall    |
...
| `/api/auth/**`             | `mall-auth`      | C 端登录              | 手机号、微信等登录     |
...
| `/api/search/**`           | `mall-search`    | 可匿名或 C 端 token   | 商品搜索               |
...
代码层统一使用 `ruoyi-gateway` 模块。生产部署可以按 profile 拆成两个运行实例：
...
这样可以让管理端和 C 端使用不同域名、证书、限流策略、网络入口和发布节奏。
...
mall:search:index:rebuild       # 重建搜索索引
mall:search:index:sync          # 触发增量同步
```
...
早期建议部署在同一个 MySQL 实例和同一个 schema 中，通过表前缀、Mapper 包、`domain/` 实体目录和迁移脚本目录隔离访问范围。
...
mall.search.index.sync                 # 商品索引增量同步
mall.search.index.rebuild              # 商品索引全量重建完成
```
...
管理端基于若依 UI 扩展商城菜单。商城页面统一放在 `views/mall/`，接口统一调用 `/admin/mall/**`。
...
```
10000-19999  common（通用）
20000-29999  auth（认证）
30000-39999  user（用户会员）
40000-49999  product（商品）
50000-59999  order（订单）
60000-69999  payment（支付）
70000-79999  marketing（营销）
80000-89999  search（搜索）
```
...
└─ src/main/java/com/jhstore/mall/order/
   ...
   │  ├─ admin/              # 管理端接口
   │  └─ api/                # C端接口
   ├─ domain/                # 数据库实体类（XxxDO，对应本服务 MySQL 表）
   ├─ service/               # 业务服务
   ├─ mapper/                # 数据访问
...
- `domain/` 只放本服务拥有的 MySQL 表对应的实体类，不放服务间 DTO、公共基类、其他服务表实体
- `mall-api` 中只放 DTO/Feign 接口/枚举，禁止放 `domain/`、`mapper/`、`service/`、`controller/`、`vo/`、`constant/`
