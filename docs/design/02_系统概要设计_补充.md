...
| `/admin/mall/user/**` | `mall-user` | 2 | 管理端 JWT + 权限码 | 商城用户管理 |
| `/admin/mall/product/**` | `mall-product` | 2 | 管理端 JWT + 权限码 | 商品后台管理 |
| `/admin/mall/order/**` | `mall-order` | 2 | 管理端 JWT + 权限码 | 订单后台管理 |
| `/admin/mall/payment/**` | `mall-payment` | 2 | 管理端 JWT + 权限码 | 支付与退款后台管理 |
| `/admin/mall/marketing/**` | `mall-marketing` | 2 | 管理端 JWT + 权限码 | 营销后台管理 |
| `/admin/mall/search/**` | `mall-search` | 2 | 管理端 JWT + 权限码 | 搜索后台管理 |
...
认证分流按路径前缀决定：`/api/**` 走 MallAuthFilter，其他非白名单位置走 AuthFilter。两个过滤器互不干扰。
