...
服务间调用只使用契约 DTO 和 Feign 客户端接口。VO 在各服务 `convert/response/` 中定义（DO→VO 出站转换），不放入 `mall-api`。Request→DO 入站转换收口于 `convert/request/`。数据库实体、Mapper、Service 实现不能放进 `mall-api`。
...
service/       # 业务逻辑实现
controller/    # 控制层
vo/            # 视图对象（在各服务 convert/response/ 中定义）
...
| **mall-api 边界**     | `mall-api` 只放跨服务契约（DTO/Feign/枚举），不放 VO 和业务常量。VO 在各服务 `convert/response/` 中定义（DO→VO 出站），Request→DO 入站收口于 `convert/request/`。避免前端展示模型与服务间契约耦合                                                                                                         |
...
- Service 不返回数据库实体（DO）给前端，必须通过 `convert/response/` 转换为 VO
- Mapper 不跨服务访问数据库表
