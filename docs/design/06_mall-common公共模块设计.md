...
C 端所有错误码统一由 `mall-common` 的 `ErrorCode` 枚举集中定义，枚举定义即唯一真相源。各模块设计文档仅列出自己特有的错误码，公共错误码以此处为准。

### 6.1 公共错误码（全集群通用）

`ErrorCode` 枚举中以下错误码由所有 C 端模块共享，在各模块设计文档中不重复列出：

| 枚举值 | 错误码 | HTTP | message | userTip |
|--------|--------|:---:|---------|---------|
| `SUCCESS` | `00000` | 200 | 成功 | 成功 |
| `NOT_AUTHENTICATED` | `A0301` | 401 | 未携带有效 token | 请先登录 |
| `TOKEN_TAMPERED` | `A0310` | 401 | token 被篡改或伪造 | 登录凭证非法 |
| `INTERNAL_SIGN_INVALID` | `A0311` | 401 | 内部请求签名校验失败 | 系统繁忙，请稍后再试 |
| `INTERNAL_SIGN_MISSING` | `A0312` | 401 | 内部请求缺少签名头 | 系统繁忙，请稍后再试 |
| `NO_PERMISSION` | `A0320` | 403 | token 有效但权限不足 | 无操作权限 |
| `PARAM_MISSING` | `A0401` | 400 | 必填参数为空 | 请完整填写信息 |
| `PARAM_INVALID` | `A0402` | 400 | 参数格式错误 | 参数格式错误 |
| `RATE_LIMIT` | `A0410` | 400 | 短信/接口频率限制 | 请求过于频繁，请稍后再试 |
| `BODY_INVALID` | `A0420` | 400 | 请求体格式错误 | 请求体格式错误 |
| `RESOURCE_NOT_FOUND` | `A0501` | 400 | 资源不存在 | 资源不存在 |
| `RESOURCE_EXISTS` | `A0502` | 400 | 资源已存在（重复创建） | 资源已存在（重复创建） |
| `RESOURCE_STATUS_ERROR` | `A0503` | 400 | 资源状态异常 | 资源状态异常 |
| `SYSTEM_ERROR` | `B0001` | 500 | 未预期异常 | 系统繁忙，请稍后再试 |
| `SYSTEM_TIMEOUT` | `B0100` | 500 | 系统执行超时 | 系统繁忙，请稍后再试 |
| `SYSTEM_CAPACITY` | `B0200` | 500 | 系统资源不足 | 系统繁忙，请稍后再试 |
| `SYSTEM_INTERNAL` | `B0300` | 500 | 空指针等未预期异常 | 系统繁忙，请稍后再试 |
| `THIRD_PARTY_ERROR` | `C0001` | 500 | 调用第三方服务出错 | 服务暂时不可用 |
| `REDIS_ERROR` | `C0110` | 500 | Redis 连接失败 | 服务暂时不可用 |
| `MYSQL_ERROR` | `C0120` | 500 | MySQL 服务出错 | 服务暂时不可用 |
| `MQ_ERROR` | `C0130` | 500 | MQ 消息服务出错 | 服务暂时不可用 |
| `NOT_OPEN` | `A9999` | 400 | 该功能暂未开放 | 该功能暂未开放 |

### 6.2 错误码添加规范

1. 新增错误码先在 `ErrorCode` 枚举中定义（`code` + `message` + `userTip`），**不再在各模块分散定义**
2. 公共错误码（跨模块使用）放入 §6.1 表
3. 模块特有错误码仅在该模块设计文档的"错误码"章节列出，并注明"其余公共错误码见 `06_mall-common` §6.1"
4. HTTP 状态码在 `ErrorCode` 枚举中显式定义（`getHttpStatus()`），`MallExceptionHandler` 通过 `ResponseEntity` 返回
