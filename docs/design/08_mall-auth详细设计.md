...
> 公共错误码（A0301/A0310/A0401/B0001 等）见 `06_mall-common公共模块设计.md` §6.1。
...
mall-auth 通过 Feign 调用 mall-user 时，由 `mall-api` 共享的 `InnerFeignRequestInterceptor`（`com.mall.api.infrastructure.security`）自动加签。引入 `mall-api` 即自动生效，无需额外配置。
