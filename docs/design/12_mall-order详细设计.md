...
| 错误码 | HTTP | userTip | 说明 |
|--------|:---:|---------|------|
| A0701 | 400 | 订单不存在 | 查询/操作订单时不存在 |
| A0702 | 400 | 订单状态异常 | 状态机转移矩阵无匹配 |
| A0703 | 400 | 订单状态不允许当前操作 | 前置条件不满足（如删除非终态订单） |
| A0710 | 400 | 购物车为空 | 下单时购物车无选中项 |
| A0711 | 400 | 超出购买数量限制 | 超过限购上限 |

> 公共错误码（A0301/A0320/B0001 等）见 `06_mall-common公共模块设计.md` §6.1。
