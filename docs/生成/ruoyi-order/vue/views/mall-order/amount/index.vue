<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="amountList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="订单 ID，与订单一对一" align="center" prop="orderId" />
      <el-table-column label="商品明细快照 JSON" align="center" prop="itemsJson" />
      <el-table-column label="优惠券使用快照 JSON" align="center" prop="couponSnapshotJson" />
      <el-table-column label="活动优惠快照 JSON" align="center" prop="promotionSnapshotJson" />
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="amountRef" :model="form" :rules="rules" label-width="100px">
        <el-row>
          <el-col :span="24">
            <el-form-item label="商品明细快照 JSON" prop="itemsJson">
              <el-input v-model="form.itemsJson" type="textarea" placeholder="请输入内容" />
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="优惠券使用快照 JSON" prop="couponSnapshotJson">
              <el-input v-model="form.couponSnapshotJson" type="textarea" placeholder="请输入内容" />
            </el-form-item>
          </el-col>
          <el-col :span="24">
            <el-form-item label="活动优惠快照 JSON" prop="promotionSnapshotJson">
...
const data = reactive({
  ...
  rules: {
    orderId: [
      { required: true, message: "订单 ID，与订单一对一不能为空", trigger: "blur" }
    ],
    itemsJson: [
      { required: true, message: "商品明细快照 JSON不能为空", trigger: "blur" }
    ],
    totalAmount: [
